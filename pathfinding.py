...
window = Tk()
label = Label(window, text='Inicio(x,y): ')
startBox = Entry(window)
label1 = Label(window, text='Fin(x,y): ')
endBox = Entry(window)
var = IntVar()
showPath = ttk.Checkbutton(window, text='Mostrar pasos?', onvalue=1, offvalue=0, variable=var)
...
def main():
    ...
    if len(openSet) > 0:
        ...
        if current == end:
            ...
            Tk().wm_withdraw()
            result = messagebox.askokcancel('Busqueda concluida', ('El programa ha terminado, la distancia más corta \n al punto es de ' + str(temp) + ' bloques, \n ¿le gustaria volver a correr el programa?'))
            if result == True:
